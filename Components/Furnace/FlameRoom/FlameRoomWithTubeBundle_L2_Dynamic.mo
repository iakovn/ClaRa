...
extends ClaRa.Components.Furnace.BaseClasses.CombustionChamberBase_Advanced(geo(
        flowOrientation=ClaRa.Basics.Choices.GeometryOrientation.vertical));
extends ClaRa.Components.Furnace.BaseClasses.CombustionChamberBase_additional_HPs;
...
equation
  ...
  Delta_h_f - LHV =m_flow_flueGas_id*((ideal_combustion.h_i)*cat(
    1,
    xi_flueGas_id,
    {1 - sum(xi_flueGas_id)})) + xi_coal_in[6]*reactionZone.xi_slag*outlet.slagType.cp*T_0;                  //formation enthalpy of used coal

  //_______________/ Energy Balance flueGasCombustion \__________________________
  der(h_flueGas_out) =(Q_flow_wall + Q_flow_top + Q_flow_bottom + Q_flow_CarrierTubes + Q_flow_TubeBundle + inlet.flueGas.m_flow*(flueGasInlet.h - h_flueGas_out) + inlet.coal.m_flow*((inlet.coalType.cp*(inStream(inlet.coal.T_outflow) - T_0) + Delta_h_f) - h_flueGas_out) + outlet.coal.m_flow*((outlet.coalType.cp*(outlet.coal.T_outflow - T_0) + Delta_h_f) - h_flueGas_out) + outlet.slag.m_flow*(outlet.slagType.cp*(inStream(outlet.slag.T_outflow) - T_0) - h_flueGas_out) + inlet.slag.m_flow*(inlet.slagType.cp*(inlet.slag.T_outflow - T_0) - h_flueGas_out) + outlet.flueGas.m_flow*(flueGasOutlet.h - h_flueGas_out))/mass;
