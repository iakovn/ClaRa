...
protected
  TILMedia.Gas_pT    primaryAir_inlet(p=coalDust_inlet.flueGas.p, T=actualStream(coalDust_inlet.flueGas.T_outflow), xi=actualStream(
...
equation
  ...
  inlet.flueGas.m_flow * flueGasInlet.xi +  coalDust_inlet.flueGas.m_flow * primaryAir_inlet.xi - (inlet.flueGas.m_flow+coalDust_inlet.flueGas.m_flow)*xi_flueGasMix = zeros(flueGas.nc-1);

  xi_flueGasMix_del = xi_flueGasMix;

  //________________/ Mass balance - flue gas \______________________________________

   drhodt*geo.volume =m_flow_coal_burned*(1 - xi_coal_in[6]*reactionZone.xi_slag) + inlet.flueGas.m_flow + coalDust_inlet.flueGas.m_flow + outlet.flueGas.m_flow;
  ...
  // theoretically required oxygen mass flow rate to burn all the coal
   ...
   m_flow_air_req*max(1e-32,primaryAir_inlet.xi[6]) = m_flow_oxygen_req;
    ...
    lambdaComb = (inlet.flueGas.m_flow*flueGasInlet.xi[6] + coalDust_inlet.flueGas.m_flow*primaryAir_inlet.xi[6])/max(1e-12, m_flow_oxygen_req);
    end if;

  //calculation of actual coal and oxygen mass flow rates that are burned
  if noEvent(lambdaComb > 1) then
    m_flow_coal_burned = (1 - coal_diffusity)*(inlet.coal.m_flow + coalDust_inlet.coal.m_flow);
    m_flow_oxygen_burned = m_flow_oxygen_req;
   else
    m_flow_coal_burned = lambdaComb*(1 - coal_diffusity)*(inlet.coal.m_flow + coalDust_inlet.coal.m_flow);
  ...
  der(xi_flueGas) =1/mass*(inlet.flueGas.m_flow*(flueGasInlet.xi - xi_flueGas) + coalDust_inlet.flueGas.m_flow*(primaryAir_inlet.xi - xi_flueGas) + outlet.flueGas.m_flow*(flueGasOutlet.xi - xi_flueGas) + m_flow_coal_burned*(prod_comp - xi_flueGas));
  ...
  der(h_flueGas_out) =(Q_flow_wall + Q_flow_top + Q_flow_bottom + inlet.flueGas.m_flow*(flueGasInlet.h - h_flueGas_out) + coalDust_inlet.flueGas.m_flow*(primaryAir_inlet.h - h_flueGas_out) + inlet.coal.m_flow*((inlet.coalType.cp*(inStream(inlet.coal.T_outflow) - T_0) + Delta_h_f) - h_flueGas_out) + coalDust_inlet.coal.m_flow*((coalDust_inlet.coalType.cp*(inStream(coalDust_inlet.coal.T_outflow) - T_0) + Delta_h_f) - h_flueGas_out) + outlet.coal.m_flow*((outlet.coalType.cp*(outlet.coal.T_outflow - T_0) + Delta_h_f) - h_flueGas_out) + outlet.slag.m_flow*(outlet.slagType.cp*(inStream(outlet.slag.T_outflow) - T_0) - h_flueGas_out) + inlet.slag.m_flow*(inlet.slagType.cp*(inlet.slag.T_outflow - T_0) - h_flueGas_out) + outlet.flueGas.m_flow*(flueGasOutlet.h - h_flueGas_out))/mass;
  ...
  m_flow_in_del = inlet.flueGas.m_flow + coalDust_inlet.coal.m_flow;
  ...
  coalDust_inlet.coal.T_outflow = bulk.T;
  coalDust_inlet.flueGas.T_outflow = bulk.T;
 ...
 //____________/ (Dummy) values for inlet_outflows \_____________
  ...
  coalDust_inlet.flueGas.xi_outflow =  xi_flueGas_del;
...
<p><b>Model description: </b>A stationary flame room model for coal fired furnaces with additional air port</p>
