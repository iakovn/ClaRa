...
model ValveCoalDust_L1 "Valve for mixed coal and flue gas flow with replaceable flow models"
...
//___________________________________________________________________________//
  ...
  ClaRa.Basics.Interfaces.CoalDust_inlet inlet(flueGas(Medium=medium),
      final coalType=coalType) "Inlet port"
    annotation (Placement(transformation(extent={{-110,-10},{-90,10}})));

  ClaRa.Basics.Interfaces.CoalDust_outlet outlet(flueGas(Medium=
          medium), final coalType=coalType) "Outlet port"
...
public
  ...
  Summary summary(
    ...
    inlet(coal(m_flow=inlet.coal.m_flow,
    ...
    outlet(coal(m_flow=-outlet.coal.m_flow,
...
// Isenthalpic state transformation (no storage and no loss of energy)
  ...
  inlet.coal.T_outflow = inStream(outlet.coal.T_outflow);
  outlet.coal.T_outflow = inStream(inlet.coal.T_outflow);
...
//_______________Mass balance (no storage)__________________________
  ...
  inlet.coal.m_flow + outlet.coal.m_flow = 0;

  pressureLoss.Delta_p =  inlet.coal.p - outlet.coal.p;
...
//______________ No chemical reaction taking place:_________________
  ...
  inlet.coal.xi_outflow = inStream(outlet.coal.xi_outflow);
  outlet.coal.xi_outflow = inStream(inlet.coal.xi_outflow);
