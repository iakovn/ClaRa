...
model TubeBundle_L1_TML "Simple tube model based on transmission line equations. Can choose between Modelica and ClaRa Delay implementation."
  ...
  parameter Modelica.SIunits.SpecificEnthalpy h_start=
      ...
      medium,
      p_start,
      simCenter.T_amb) "|Initialisation|Initial Medium Properties|Initial averaged fluid specific enthalpy";
  ...
  parameter Modelica.SIunits.Pressure p_in_start=simCenter.p_amb "|Initialisation|Initial Medium Properties|Initial inlet pressure";
  parameter Modelica.SIunits.Pressure p_out_start=simCenter.p_amb "|Initialisation|Initial Medium Properties|Initial outlet pressure";
...
protected
  ...
  Real Alpha;
  Real Beta;
  Real I_f[N_temp - 1](stateSelect=StateSelect.never);
  ...
  Real hist_T[N_temp, 1](start=ones(N_temp, 1)*simCenter.T_amb);
