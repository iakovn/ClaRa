...
model HEXvle2gas_L3_2ph_BU_simple "VLE 2 gas | L3 | 1 phase on each side | Block shape | U-type |"
  ...
  replaceable model HeatTransferTubes =
    ...
    constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.VLE_HT.HeatTransfer_L3 "Heat transfer mode at the tubes side"
  ...
  replaceable model PressureLossTubes =
    ...
    constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.Generic_PL.PressureLoss_L3 "Pressure loss model at the tubes side"
                                            annotation (Dialog(tab="Tubes",
        group="Geometry"), choicesAllMatching);
  parameter ClaRa.Basics.Units.Length
  ...
  Basics.ControlVolumes.FluidVolumes.VolumeVLE_3_TwoZones
    ...
    redeclare model Geometry =
        ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.PipeWithHotwell (
        z_in={z_in_tubes},
        z_out={z_out_tubes},
        diameter=diameter_i,
        N_tubes=N_tubes,
...
public
  Basics.ControlVolumes.SolidVolumes.ThinWall_L2 wall(
    thickness_wall=diameter_o - diameter_i,
    ...
    A_heat=tubes.geo.A_heat[tubes.heattransfer.heatSurfaceAlloc]) annotation (Placement(transformation(extent={{20,24},{40,34}})));

  parameter Real level_rel_start=0.5 "|Shell Side|Initialisation|Start value for relative filling Level";
  parameter Basics.Units.Length height_hotwell=1 "|Tube Side|Geometry|Height of the hotwell";
  parameter Basics.Units.Length width_hotwell=1 "|Tube Side|Geometry|Width of the hotwell";
  parameter Basics.Units.Length length_hotwell=1 "|Tube Side|Geometry|Length of the hotwell";
equation
...
equation
  ...
  connect(shell.heat, wall.outerPhase) annotation (Line(
      points={{10,60},{30,60},{30,34}},
      color={167,25,48},
      thickness=0.5,
      smooth=Smooth.None));
  connect(tubes.heat, wall.innerPhase) annotation (Line(
      points={{50,1.77636e-015},{50,0},{30,0},{30,24}},
      color={167,25,48},
      thickness=0.5,
      smooth=Smooth.None));
