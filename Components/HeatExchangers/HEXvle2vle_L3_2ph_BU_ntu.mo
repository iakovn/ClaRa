...
//     annotation (Dialog(tab="Shell Side", group="Initialisation"));
  ...
  Basics.ControlVolumes.FluidVolumes.VolumeVLE_2 tubes(
    ...
    redeclare model Geometry =
        ...
        N_tubes=N_tubes,
        N_passes=N_passes,
        length=if shell.geo.flowOrientation==ClaRa.Basics.Choices.GeometryOrientation.vertical then if parallelTubes then height else length else if parallelTubes then length else width),
    showExpertSummary=showExpertSummary)
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=90,
        origin={80,0})));
