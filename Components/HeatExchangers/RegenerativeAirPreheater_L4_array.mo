...
model RegenerativeAirPreheater_L4_array "Model for a regenerative air preheater"
...
//                                                                                                     annotation (choicesAllMatching);
  ...
  final parameter ClaRa.Basics.Units.Area A_flueGas_free=A_flueGas*(1 -
      f_plates);
  ...
  //Fresh air volume
  final parameter ClaRa.Basics.Units.Volume volume_reg_eff=(A_cross - A_hub -
      A_covered)*height_reg;
  //Effective regenerator volume (without hub and covered volume)
  final parameter ClaRa.Basics.Units.Volume volume_st=(A_cross - A_hub)*(f_plates)*
      height_reg;
  //Volume of solid regenerator storage material
  final parameter ClaRa.Basics.Units.Mass mass=if calculate_mass then volume_st*solid.d
       else mass_fixed;
  //Mass of regenerator storage material
  final parameter ClaRa.Basics.Units.Length d_gl=4*(A_cross - A_hub - A_plates)
      /(2*N_sp*b);
  //Equivalent diameter
protected
...
public
  ...
  ClaRa.Basics.Interfaces.GasPortIn flueGasInlet(Medium=medium, m_flow(min=-Modelica.Constants.inf)) "Inlet port"
        ...
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={46,-60}), iconTransformation(extent={{90,-70},{110,-50}})));
  ...
  ClaRa.Basics.Interfaces.GasPortIn freshAirInlet(Medium=medium, m_flow(min=-Modelica.Constants.inf)) "Inlet port"
        ...
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-46,80}), iconTransformation(extent={{-110,50},{-90,70}})));
  ...
  Basics.ControlVolumes.GasVolumes.FlueGasCell_L4 flueGasCell(
    ...
    redeclare model Geometry = ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.GenericGeometry_N_cv (
        ...
        A_heat=ones(N_cv, flueGasCell.heatSurfaceAlloc)*A_heat/N_cv)) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={46,0})));

        //(A_cross=ones(N_cv)*A_flueGas_free,diameter_hyd=ones(N_cv)*d_gl, A_heat=ones(N_cv)*A_heat/N_cv)

  Basics.ControlVolumes.GasVolumes.FlueGasCell_L4 freshAirCell(
    ...
    redeclare model Geometry = ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.GenericGeometry_N_cv (
        ...
        A_heat=ones(N_cv, freshAirCell.heatSurfaceAlloc)*A_heat/N_cv)) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-46,0})));

     //(A_cross=ones(N_cv)*A_air_free,diameter_hyd=ones(N_cv)*d_gl, A_heat=ones(N_cv)*A_heat/N_cv)
...
equation
  ...
  connect(freshAirInlet, split_controllable.inlet) annotation (Line(
      points={{-46,80},{-46,40.6667},{-44,40.6667}},
      color={118,106,98},
      thickness=0.5,
      smooth=Smooth.None));
  ...
  for i in 1:(N_cv) loop
    ...
    connect(wallSecundaryAir[i].outerPhase, flueGasCell.heat[N_cv + 1 - i])
      annotation (Line(
        points={{4,-1.77636e-015},{22,-1.77636e-015},{22,6.66134e-016},{36.4,6.66134e-016}},
        color={167,25,48},
        thickness=0.5,
        smooth=Smooth.None));
...
end RegenerativeAirPreheater_L4_array;
