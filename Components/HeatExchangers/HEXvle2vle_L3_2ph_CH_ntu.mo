...
model HEXvle2vle_L3_2ph_CH_ntu "VLE 2 VLE | L3 | 2 phase at shell side | Cylinder shape | Header type | simple HT"
  ...
  parameter Basics.Units.Temperature T_w_tube_start[3]=ones(3)*293.15 "|Wall|Initialisation|Initial temperature at inner phase";
  parameter Basics.Units.Temperature T_w_shell_start[3]=ones(3)*293.15 "|Wall|Initialisation|Initial temperature at outer phase";
...
equation
  connect(tubes.inlet, In2) annotation (Line(
      points={{48,0},{74,0},{74,20},{100,20}},
      color={0,131,169},
      thickness=0.5,
      smooth=Smooth.None));
  connect(tubes.outlet, Out2) annotation (Line(
      points={{28,0},{-36,0},{-36,20},{-100,20}},
      color={0,131,169},
      thickness=0.5,
      smooth=Smooth.None));
