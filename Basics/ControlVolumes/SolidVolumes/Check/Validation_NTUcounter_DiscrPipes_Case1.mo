...
//___________________________________________________________________________//
  ...
  parameter Units.Temperature
                           T_i_in = 100+273.15 "Temperature of cold side";
  parameter Units.Temperature
                           T_o_in = 300+273.15 "Temperature of hot side";
  parameter Units.MassFlowRate
                            m_flow_i = 10 "Mass flow of cold side";
  parameter Units.MassFlowRate
                            m_flow_o = 100 "Mass flow of hot side";
  parameter Units.Pressure
                        p_i = 2e5 "Pressure of cold side";
  parameter Units.Pressure
                        p_o = 300e5 "Pressure of hot side";

  parameter Units.CoefficientOfHeatTransfer
                                         alpha_i = 730 "Heat transfer coefficient of cold side";
  parameter Units.CoefficientOfHeatTransfer
                                         alpha_o = 7300 "Heat transfer coefficient of hot side";
  ...
  parameter Integer N_cv = 100;//400 "Number of Cells";
  ...
  parameter Units.EnthalpyMassSpecific
    ...
    Units.HeatFlowRate
                    Q_flow_tot;
    Units.HeatCapacityMassSpecific
                                cp_o_m;
    Units.HeatCapacityMassSpecific
                                cp_i_m;
    Units.HeatCapacityMassSpecific
  ...
  Components.VolumesValvesFittings.Pipes.PipeFlow_L4_Simple pipe_OuterSide(
    length=length,
    N_tubes=N_tubes,
    N_cv=N_cv,
  ...
  Components.VolumesValvesFittings.Pipes.PipeFlow_L4_Simple pipe_InnerSide(
    length=length,
    N_tubes=N_tubes,
    N_cv=N_cv,
    Delta_x=ones(N_cv)*length/N_cv,
  ...
  ClaRa.Components.Sensors.Temperature  OuterSide_outletTemp
    annotation (Placement(transformation(extent={{-42,-20},{-22,0}})));
  ClaRa.Components.Sensors.Temperature  InnerSide_outletTemp
  ...
  Visualisation.Hexdisplay_3 hexdisplay_3_1(
    ...
    yps_i=wall_NTU.summary.eCom.z_i) annotation (Placement(transformation(extent={{46,-88},{158,16}})));
...
equation
  for i in 1:pipe_InnerSide.N_cv loop
    ...
    connect(pipe_InnerSide.heat[i], thinWall.innerPhase[(pipe_InnerSide.N_cv +
      1) - i]);
 ...
 Cell_hv=integer(Modelica.Math.Vectors.find(val,x,15e3));
  ...
  connect(InnerSide_outletTemp.port, InnerSide_out.steam_a)
      ...
      smooth=Smooth.None));
  annotation (                                                        Diagram(
        coordinateSystem(extent={{-160,-100},{160,100}}, preserveAspectRatio=false),
        graphics={Text(
          extent={{-158,98},{162,90}},
          lineColor={0,128,0},
          textStyle={TextStyle.Bold},
          horizontalAlignment=TextAlignment.Left,
          textString="Validated! // 08. Jan 2013, FG"),
                                                   Text(
...
LOOK AT:
Outlet temperature of cooled liquid: OuterSide_outletTemp.T  vs. wall_NTU.summary.T_o[4]   = 248.31 °C  vs 248.29 °C
Outlet temperature of evaporated water: InnerSide_outletTemp.T vs wall_NTU.summary.T_i[4]   = 215.55 °C vs 216.11 °C
___________________________________________________________________________________________________
NOTE:
