...
public
  ...
  replaceable model PressureLoss =
      ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.Generic_PL.LinearPressureLoss_L4            constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.PressureLossBaseVLE_L4 "|Fundamental Definitions|Pressure loss model at the tubes side"
                                                                                            annotation (choicesAllMatching);

  replaceable model HeatTransfer =
      ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.Generic_HT.CharLine_L4                     constrainedby ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.HeatTransferBaseVLE_L4 "|Fundamental Definitions|Heat transfer mode at the tubes side"
                                                                                            annotation (choicesAllMatching);
...
public
  ...
  parameter Basics.Units.EnthalpyMassSpecific h_nom[geo.N_cv]=ones(geo.N_cv)*1e5 "|Nominal Values|Nominal specific enthalpy for single tube";
  inner parameter Basics.Units.MassFlowRate m_flow_nom=100 "|Nominal Values|Nominal mass flow w.r.t. all parallel tubes";
  inner parameter Basics.Units.PressureDifference Delta_p_nom=1e4 "|Nominal Values|Nominal pressure loss w.r.t. all parallel tubes";
  ...
  parameter ClaRa.Basics.Choices.Init initType=ClaRa.Basics.Choices.Init.steadyState "|Initialisation||type of initialisation " annotation (choicesAllMatching);
  ...
  parameter Basics.Units.EnthalpyMassSpecific h_start[geo.N_cv]=ones(geo.N_cv)*800e3 "|Initialisation||Initial specific enthalpy for single tube";
  parameter Basics.Units.Pressure p_start[geo.N_cv]=ones(geo.N_cv)*1e5 "|Initialisation||Initial pressure";
...
protected
  inner TILMedia.VLEFluid_ph fluid[geo.N_cv](
    ...
    each computeTransportProperties=true) annotation (Placement(transformation(extent={{-10,-40},{10,-20}}, rotation=0)));
