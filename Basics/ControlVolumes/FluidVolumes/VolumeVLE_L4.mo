...
//## S U M M A R Y   D E F I N I T I O N #######################################################################
  record Outline
    ...
    input Basics.Units.Momentum I[N_cv+1] if  showExpertSummary "Momentum of fluid flow volumes through cell borders"
...
public
 parameter Basics.Units.Pressure
                       p_nom[geo.N_cv]= ones(geo.N_cv)*1e5 "|Nominal Values|Nominal pressure";
 ...
 inner parameter Basics.Units.PressureDifference
...
public
parameter Boolean showExpertSummary=true "|Summary and Visualisation||True, if an extended summary shall be shown, else false";
...
//____Pressure__________________________________________________________________________________________________
 Basics.Units.Pressure
             p[geo.N_cv](start=p_start_internal) "Cell pressure";
 Basics.Units.PressureDifference
             Delta_p_fric[geo.N_cv+1] "Pressure difference due to friction";
 Basics.Units.PressureDifference
...
//____Mass and Density__________________________________________________________________________________________
  ...
  Basics.Units.Mass
          mass_FM[geo.N_cv+1]=cat(1,{mass[1]/2},{(mass[i]+mass[i-1])/2 for i in 2:geo.N_cv},{mass[geo.N_cv]/2}) "Mass of fluid in flow cells";
  Real drhodt[geo.N_cv];//(unit="kg/(m3s)")
  Basics.Units.MassFraction
                  steamQuality[geo.N_cv] "Steam fraction";
  Basics.Units.MassFraction
                  steamQuality_inlet "Steam fraction";
  Basics.Units.MassFraction
                  steamQuality_outlet "Steam fraction";
...
//     annotation(Placement(transformation(extent={{140,-50},
//             {160,-30}}), iconTransformation(extent={{94,-66},{114,-46}})));
...
public
  ...
  inner TILMedia.VLEFluid_ph  fluidOutlet(
    ...
    h=actualStream(outlet.h_outflow))                         annotation (Placement(transformation(extent={{70,-30},
            {90,-10}},                                                                                                   rotation=0)));
...
protected
  ...
  inner TILMedia.VLEFluid_ph  fluidInlet(
    p=inlet.p,
    each vleFluidType =    medium,
    h=actualStream(inlet.h_outflow))                          annotation (Placement(transformation(extent={{-90,-30},
            {-70,-10}},                                                                                                  rotation=0)));
...
//pressure differece due to gravity
    Delta_p_grav[i]=(fluid[i].d*geo.Delta_x[i]+fluid[i-1].d*geo.Delta_x[i-1])/2*g_n*(geo.z_out-geo.z_in)/geo.length;
  end for;

  Delta_p_fric[geo.N_cv+1]=pressureLoss.Delta_p[geo.N_cv+1];
  Delta_p_fric[1]=pressureLoss.Delta_p[1];

  Delta_p_grav[1]=fluid[1].d*geo.Delta_x[1]/2*g_n*(geo.z_out-geo.z_in)/geo.length;
  Delta_p_grav[geo.N_cv+1]=fluid[geo.N_cv].d*geo.Delta_x[geo.N_cv]/2*g_n*(geo.z_out-geo.z_in)/geo.length;
...
if not frictionAtInlet and not frictionAtOutlet then //no friction pressure loss inlet->first cell / no friction pressure loss last cell->outlet
  inlet.p = fluid[1].p+Delta_p_grav[1];
  outlet.p= fluid[geo.N_cv].p-Delta_p_grav[geo.N_cv+1];
...
elseif frictionAtInlet and not frictionAtOutlet then //friction pressure loss inlet->first cell / no friction pressure loss last cell->outlet
  0 =    if useHomotopy then homotopy(inlet.p - p[1] - Delta_p_fric[1] -Delta_p_grav[1],inlet.p - p[1] - Delta_p_fric[1] -Delta_p_grav[1])
                                 else inlet.p - p[1] - Delta_p_fric[1] -Delta_p_grav[1];
  outlet.p= fluid[geo.N_cv].p-Delta_p_grav[geo.N_cv+1];
...
elseif  not frictionAtInlet and frictionAtOutlet then//"no friction pressure loss inlet->first cell / friction pressure loss last cell->outlet"
  0 =    if useHomotopy then homotopy(p[geo.N_cv] - outlet.p - Delta_p_fric[geo.N_cv+1] -Delta_p_grav[geo.N_cv+1],p[geo.N_cv] - outlet.p - Delta_p_fric[geo.N_cv+1] -Delta_p_grav[geo.N_cv+1])
                                 else p[geo.N_cv] - outlet.p - Delta_p_fric[geo.N_cv+1] -Delta_p_grav[geo.N_cv+1];
  inlet.p = fluid[1].p+Delta_p_grav[1];
