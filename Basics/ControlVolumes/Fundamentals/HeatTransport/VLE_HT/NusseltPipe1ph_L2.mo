...
public
  ...
  final parameter FluidDissipation.HeatTransfer.StraightPipe.kc_overall_IN_con inCon_1ph(
    ...
    d_hyd=geo.diameter) annotation (Placement(transformation(extent={{34,-100},{54,-80}})));
  ...
  final parameter FluidDissipation.HeatTransfer.StraightPipe.kc_overall_IN_var inVar_1ph_nom(
    ...
    rho=fluidFunction_rho(
        ...
        iCom.h_nom,
        iCom.xi_nom),
    m_flow=noEvent(abs(iCom.m_flow_nom/geo.N_tubes))) annotation (Placement(transformation(extent={{60,-70},{80,-50}})));
  FluidDissipation.HeatTransfer.StraightPipe.kc_overall_IN_var inVar_1ph(
    ...
    lambda=max(1e-10, fluidObjectFunction_lambda(
        iCom.p_out,
        iCom.h_out,
        iCom.xi_out,
        iCom.fluidPointer_out)),
    ...
    eta=fluidObjectFunction_eta(
        ...
        iCom.xi_out,
        iCom.fluidPointer_out),
    m_flow=noEvent(max(0.0000001, abs(iCom.m_flow_in/geo.N_tubes)))) annotation (Placement(transformation(extent={{60,-100},{80,-80}})));
...
equation
  ...
  //Index 1 for shell surface

  // defining the HTC for a straight pipe for ONE PHASE FLOW:

//  alpha = CF_alpha_tubes*(if useHomotopy then homotopy(noEvent(min(1e4*inVar_1ph_nom.lambda/inCon_1ph.d_hyd, FluidDissipation.HeatTransfer.StraightPipe.kc_overall(inCon_1ph, inVar_1ph))), alpha_nom*abs(sum(iCom.m_flow_in)/geo.N_tubes)/sum(iCom.m_flow_nom)/geo.N_tubes) else noEvent(min(1e4*inVar_1ph_nom.lambda/inCon_1ph.d_hyd, FluidDissipation.HeatTransfer.StraightPipe.kc_overall(inCon_1ph, inVar_1ph))));
  alpha = CF_alpha_tubes*(if useHomotopy then homotopy(FluidDissipation.HeatTransfer.StraightPipe.kc_overall(inCon_1ph, inVar_1ph), alpha_nom*abs(sum(iCom.m_flow_in)/geo.N_tubes)/sum(iCom.m_flow_nom)/geo.N_tubes) else noEvent(min(1e4*inVar_1ph_nom.lambda/inCon_1ph.d_hyd, FluidDissipation.HeatTransfer.StraightPipe.kc_overall(inCon_1ph, inVar_1ph))));
