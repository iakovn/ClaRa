...
model NusseltPipe2ph_L2 "Pipe Geo || L2 || HTC || Nusselt || 2ph"
  ...
  parameter SI.HeatFlowRate Q_flow_nom=1e6 "Nominal heat flow rate" annotation (Dialog(group="Nominal Values"));
protected
...
//   parameter SI.CoefficientOfHeatTransfer alpha_nom=(if inVar_2ph_nom.x_flow >= (1 - eps) or inVar_2ph_nom.x_flow <= 1e-3 then kc_overall(inCon_1ph, inVar_1ph_nom) else kc_twoPhaseOverall_KC(inCon_2ph, inVar_2ph_nom));
  parameter SI.CoefficientOfHeatTransfer alpha_nom=kc_overall(inCon_1ph, inVar_1ph_nom);
