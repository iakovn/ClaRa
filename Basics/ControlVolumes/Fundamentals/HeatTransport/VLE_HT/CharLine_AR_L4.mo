...
model CharLine_AR_L4 "AR Heat transfer coefficient defined by a characteristic line and a nominal value"
...
import Modelica.Constants.eps;
  ...
  parameter Real PL_kc[:,2] = {{0,0.2},{0.5,0.6},{0.7,0.72},{1,1}} "Correction factor for heat transfer in part load"
                                                       annotation(Dialog(group="Heat Transfer"));
...
ClaRa.Basics.Units.TemperatureDifference Delta_T_wi[iCom.N_cv] "Temperature difference between wall and fluid inlet temperature";
  ClaRa.Basics.Units.TemperatureDifference
                 Delta_T_wo[iCom.N_cv] "Temperature difference between wall and fluid outlet temperature";
  ClaRa.Basics.Units.TemperatureDifference
                 Delta_T_mean[iCom.N_cv];
...
equation
  ...
  Delta_T_mean[i] = noEvent(if abs(Delta_T_wo[i])<=1e-6 or abs(Delta_T_wi[i])<=1e-6 then 0
        elseif
              (heat[i].T<fluid[i].T and heat[i].T>fluid[i-1].T) or (heat[i].T>fluid[i].T and heat[i].T<fluid[i-1].T) then 0 elseif abs(Delta_T_wo[i]-Delta_T_wi[i])<=eps  then Delta_T_wi[i]
        else  (Delta_T_u[i]-Delta_T_l[i])/log(Delta_T_u[i]/Delta_T_l[i]));
  ...
  Delta_T_mean[1] = noEvent(if abs(Delta_T_wo[1])<=1e-6 or abs(Delta_T_wi[1])<=1e-6 then 0
        elseif
              (heat[1].T<fluid[1].T and heat[1].T>fluidInlet.T) or (heat[1].T>fluid[1].T and heat[1].T<fluidInlet.T) then 0 elseif abs(Delta_T_wo[1]-Delta_T_wi[1])<=eps  then Delta_T_wi[1]
        else  (Delta_T_u[1]-Delta_T_l[1])/log(Delta_T_u[1]/Delta_T_l[1]));
  ...
  Delta_T_mean[iCom.N_cv] = noEvent(if abs(Delta_T_wo[iCom.N_cv])<=1e-6 or abs(Delta_T_wi[iCom.N_cv])<=1e-6 then 0
        elseif
              (heat[iCom.N_cv].T<fluid[iCom.N_cv].T and heat[iCom.N_cv].T>fluidOutlet.T) or (heat[iCom.N_cv].T>fluid[iCom.N_cv].T and heat[iCom.N_cv].T<fluidOutlet.T) then 0 elseif abs(Delta_T_wo[iCom.N_cv]-Delta_T_wi[iCom.N_cv])<=eps  then Delta_T_wi[iCom.N_cv]
  else  (Delta_T_u[iCom.N_cv]-Delta_T_l[iCom.N_cv])/log(Delta_T_u[iCom.N_cv]/Delta_T_l[iCom.N_cv]));
...
//heat.Q_flow = alpha.*A_heat.*(heat.T-T_mean);
  ...
  for i in 1:iCom.N_cv loop
    ...
    when initial() then
       tableID=tableInit("NoName","NoName", PL_kc, smoothness);
     end when;
