...
//___________________________________________________________________________//
  ...
  parameter Real emissivity_source_fixed=0.9 "Fixed value for source emissivity"
                                        annotation (Dialog(enable=(
          suspension_calculationType == "Fixed"), group=
          "Emissivity and absorbance factor calculation of the suspension volume"));

//   parameter String emissivity_sink_calculationType="Fixed" "Calculation type"
//     annotation (Dialog(group=
//           "Emissivity and absorbance factor calculation of the gas volume"),
//       choices(choice="Fixed" "Use fixed value for sink emissivity", choice=
//           "Calculated" "Calculate sink emissivity according to VDI Waermeatlas"));

  parameter Real emissivity_sink_fixed=0.9 "Fixed value for sink emissivity"
...
//   parameter String absorbance_source_calculationType="Fixed" "Calculation type"
//     annotation (Dialog(group=
//           "Emissivity and absorbance factor calculation of the gas volume"),
//       choices(choice="Fixed" "Use fixed value for sink absorbance", choice=
//           "Calculated" "Calculate gas absorbance according to VDI Waermeatlas"));

  parameter Real absorbance_source_fixed=0.9 "Fixed value for source absorbance"
                                        annotation (Dialog(enable=(
          suspension_calculationType == "Fixed"), group=
          "Emissivity and absorbance factor calculation of the suspension volume"));
  ...
  parameter Real emissivity_particle_source=0.2 "Value considering dust particles if emissivity and absorbance are calculated"
    annotation (Dialog(enable=(
          suspension_calculationType == "Gas calculated, particles fixed"),group=
          "Emissivity and absorbance factor calculation of the suspension volume"));
  ...
  parameter Real Q_m_abs_ash = 0.2 "Mean relative cross section for absorption of ash particles"
    annotation (Dialog(enable=(suspension_calculationType == "Calculated"),
        group="Ash particle properties"));

public
  Units.Temperature T_mean;
  Real emissivity_source;
  Real emissivity_sink;
  Real absorbance_source;
  Real emissivity_H2O_CO2_source;
  Real emissivity_H2O_CO2_sink;
  Real absorbance_H2O_CO2_source;
  Real s_gl; //equivalent thickness

protected
  ClaRa.Basics.Units.Temperature T_source;
  ClaRa.Basics.Units.Temperature T_sink;
  Real view_factor;
  Real x;
  Real y;
  Real a1_source;
...
equation
 ...
 //                       + (iCom.m_flow_in*iCom.xi_in - iCom.m_flow_out*iCom.xi_out);
  ...
  smooth = Stepsmoother(5,-5,(heat.T-T_mean));
 ...
 y = if geo.flowOrientation == ClaRa.Basics.Choices.GeometryOrientation.vertical
  ...
  view_factor = (2/(Modelica.Constants.pi*x*y)*(1/2*log((1 + x^2)*(1 + y^2)/(1
             + x^2 + y^2)) + x*sqrt(1 + y^2)*atan(x/sqrt(1 + y^2)) + y*sqrt(1 + x^2)*
              atan(y/sqrt(1 + x^2)) - x*atan(x) - y*atan(y)));
  ...
  if suspension_calculationType == "Fixed" then
    ...
    heat.Q_flow = smooth*(geo.A_front*view_factor*Modelica.Constants.sigma*emissivity_sink/(absorbance_source + emissivity_sink - absorbance_source*emissivity_sink)*(emissivity_source*T_source^4 - absorbance_source*T_sink^4))
                  + (1-smooth)*(geo.A_front*view_factor*Modelica.Constants.sigma*emissivity_sink/(absorbance_source + emissivity_sink - absorbance_source*emissivity_sink)*(emissivity_source*T_source^4 - absorbance_source*T_sink^4));
  ...
  elseif suspension_calculationType == "Gas calculated, particles fixed" then
    ...
    emissivity_H2O_CO2_source = a1_source*(1 - exp(0*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl)) + a2_source*(1 - exp(-0.824*(properties.p_i[3]
                               + properties.p_i[8])/1e5*s_gl)) + a3_source*(1 - exp(-25.91*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl));

    emissivity_source = emissivity_H2O_CO2_source + emissivity_particle_source
                       - emissivity_H2O_CO2_source*emissivity_particle_source;

    //___________/ Absorbance of source volume is calculated with sink temperature (see VDI Waermeatlas)\___________________
    absorbance_H2O_CO2_source = a1_sink*(1 - exp(0*(properties.p_i[3] +
                                properties.p_i[8])/1e5*s_gl)) + a2_sink*(1 - exp(-0.824*(properties.p_i[3] +
                                properties.p_i[8])/1e5*s_gl)) + a3_sink*(1 - exp(-25.91*(properties.p_i[3] +
                                properties.p_i[8])/1e5*s_gl));

    absorbance_source = absorbance_H2O_CO2_source + emissivity_particle_source
                       - absorbance_H2O_CO2_source*emissivity_particle_source;

  //________________________/ Calculation of sink emissivity \_____________________
    emissivity_H2O_CO2_sink = a1_sink*(1 - exp(0*(properties.p_i[3] +
                              properties.p_i[8])/1e5*s_gl)) + a2_sink*(1 - exp(-0.824*(properties.p_i[3] +
                              properties.p_i[8])/1e5*s_gl)) + a3_sink*(1 - exp(-25.91*(properties.p_i[3] +
                              properties.p_i[8])/1e5*s_gl));

    emissivity_sink = emissivity_H2O_CO2_sink + emissivity_particle_sink -
                      emissivity_H2O_CO2_sink*emissivity_particle_sink;
  ...
  elseif suspension_calculationType == "Calculated" then
    ...
    emissivity_H2O_CO2_source = a1_source*(1 - exp(0*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                                + a2_source*(1 - exp(-0.824*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                                + a3_source*(1 - exp(-25.91*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl));

    //___________/ Absorbance of source volume is calculated with sink temperature (see VDI Waermeatlas)\___________________
    absorbance_H2O_CO2_source = a1_sink*(1 - exp(0*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                               + a2_sink*(1 - exp(-0.824*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                               + a3_sink*(1 - exp(-25.91*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl));

    //________________________/ Calculation of sink emissivity \_____________________
    emissivity_H2O_CO2_sink = a1_sink*(1 - exp(0*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                              + a2_sink*(1 - exp(-0.824*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl))
                              + a3_sink*(1 - exp(-25.91*(properties.p_i[3] + properties.p_i[8])/1e5*s_gl));
    ...
    heat.Q_flow = smooth*(geo.A_front*view_factor*Modelica.Constants.sigma*
                 ( emissivity_S_1 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_1)) * (a1_source * T_source^4 - a1_sink * T_sink^4)
                 + emissivity_S_2 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_2)) * (a2_source * T_source^4 - a2_sink * T_sink^4)
                 + emissivity_S_3 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_3)) * (a3_source * T_source^4 - a3_sink * T_sink^4)))
                 + (1-smooth)*(-geo.A_front*view_factor*Modelica.Constants.sigma*
                 ( emissivity_S_1 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_1)) * (a1_source * T_source^4 - a1_sink * T_sink^4)
                 + emissivity_S_2 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_2)) * (a2_source * T_source^4 - a2_sink * T_sink^4)
                 + emissivity_S_3 * emissivity_source/ (1 - (1 - emissivity_source)*(1 - emissivity_S_3)) * (a3_source * T_source^4 - a3_sink * T_sink^4)));
