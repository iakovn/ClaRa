...
Real Q_boiler;
  ...
  ClaRa.SubSystems.Boiler.ConvectiveHeatingPart_4SH SH(
    N_ax=n_Axial,
    m_flow_nom=783,
    ...
    redeclare model HeatTransfer_SH2 =
        ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.Generic_HT.CharLine_L4 (                      alpha_nom=5000),
    redeclare model HeatTransfer_SH3 =
        ClaRa.Basics.ControlVolumes.Fundamentals.HeatTransport.Generic_HT.CharLine_L4 (                      alpha_nom=5000),
  ...
  Components.BoundaryConditions.BoundaryVLE_hxim_flow  massFlowSource_XRG(m_flow_const=783, h_const=2843e3) annotation (Placement(transformation(extent={{-60,-48},{-40,-28}})));
  Components.BoundaryConditions.BoundaryVLE_phxi      pressureSink_XRG(h_const=
        3500e3, p_const=28400000)
    annotation (Placement(transformation(extent={{64,50},{40,72}})));
  Components.VolumesValvesFittings.Valves.ValveVLE_L1    valve_XRG(redeclare model PressureLoss = ClaRa.Components.VolumesValvesFittings.Valves.Fundamentals.LinearNominalPoint (m_flow_nom=824.34))
    annotation (Placement(transformation(extent={{6,54},{26,66}})));

  Components.BoundaryConditions.BoundaryVLE_phxi      pressureSink_XRG2(h_const=
        1363e3, p_const=32000000)
    annotation (Placement(transformation(extent={{102,9},{78,31}})));
  Components.BoundaryConditions.BoundaryVLE_phxi      pressureSink_XRG3(h_const=
        1363e3, p_const=32000000)
    annotation (Placement(transformation(extent={{102,-10},{78,12}})));
...
protected
  ClaRa.Basics.Interfaces.EmptyBus SetValues
...
public
  ...
  Components.Utilities.Blocks.Noise noise1(
    Tau_sample=600,
    varMeanValue=true,
    stdDev_const=1e6)
    annotation (Placement(transformation(extent={{-98,2},{-78,22}})));
  Components.Utilities.Blocks.Noise noise2(
    Tau_sample=600,
    varMeanValue=true,
    stdDev_const=1e6)
    annotation (Placement(transformation(extent={{-98,-26},{-78,-6}})));
  Components.Utilities.Blocks.TimeExtrema timeExtrema[3]
  ...
  Components.Utilities.Blocks.Noise noise3(
    Tau_sample=600,
    varMeanValue=true,
    stdDev_const=1e6)
  ...
  Components.Control.PowerPlantControl.LiveSteamTemperature liveSteamTemperature(
    k_PID2=0.1,
    k_PID1=0.1,
    Tau_i_PID1=500,
    T_a2_ref=600,
